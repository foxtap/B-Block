...
- **系统托盘驻留**：解锁后驻留托盘，关闭主窗口不退出；自动锁定 =「用完即锁」（默认：文件夹不再被使用即自动上锁，另有空闲兜底强制锁），全退出后所有文件夹回到上锁态。
...
- **扫描恢复兜底**：门户误删、文件夹被移动、甚至**配置文件丢失**导致软件里「看不到记录」时，
  可用「扫描恢复」按 NTFS 上锁特征找回；**支持指定任意目录递归扫描**（v2.10.7 起，最多下钻 6 层）。
- **状态自检 + 完成解锁**：自动比对配置状态与磁盘实况，识别「记已解锁却仍上锁」（上次解锁中断）
  与「记已上锁却没锁住」（保护失效）两类账实不符，并可一键**完成未成功的解锁**（v2.10.7 起）。
...
<p align="center"><img src="image/使用向导.png" width="400" alt="首次使用向导"></p>

### 2. 主界面

主界面提供三大入口：**开始保护（加密文件夹）**、**已加密文件列表** 与 **设置**：

<p align="center"><img src="image/首页.png" width="400" alt="主界面"></p>
...
<p align="center"><img src="image/主要功能.png" width="400" alt="主要功能"></p>
...
「用完即锁」自动锁定**默认常开**：文件夹不再被使用即自动上锁（可设宽限分钟数，0 = 用完立即锁）；
另可设「键鼠无操作 M 分钟」兜底强制锁（0 = 关闭），以及资源管理器右键菜单集成：

<p align="center"><img src="image/设置功能.png" width="400" alt="设置功能"></p>
...
如果遇到「加密 / 解锁过程中断」导致文件夹被隐藏、门户 `.lnk` 消失、软件里也看不到记录，
可以用 `recovery-tool/` 里的脚本把文件夹救回来（**只恢复权限，不删除任何文件**）：

| 文件 | 说明 |
|---|---|
| `recovery-tool/一键恢复.bat` | 双击即用，菜单式：① 扫描电脑找可疑文件夹 ② 指定路径恢复 |
| `recovery-tool/B-Block文件恢复.ps1` | 主脚本，支持 `-Mode Scan` / `-Mode Recover -Path "路径" -Apply` |
| `recovery-tool/使用说明.md` | 3 步操作 + 原理说明 + 失败处理 |

> 脚本默认**只预演、不改动磁盘**，确认后才执行；只用 Windows 自带命令（`icacls` / `attrib`），
> 无需联网、无需安装任何依赖、也不需要管理员权限（恢复的是你自己的文件夹）。
...
*本仓库发布构建产物（release/）、界面截图（image/）与紧急恢复工具（recovery-tool/）；源码不在此仓库。*
